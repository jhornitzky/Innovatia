...
function getDashIdeas($user, $limitString) {
	return dbQuery("SELECT * FROM Ideas WHERE userId='$user' ORDER BY lastUpdateTime $limitString ");
}
...
function getDashCompare($user, $limitString) {
	...
	WHERE RiskEvaluation.ideaId = Ideas.ideaId AND Ideas.userId='$user' ORDER BY lastUpdateTime $limitString");
}
...
function getDashSelect($userid, $limitString) {
	return dbQuery("SELECT Ideas.*, Selections.selectionId FROM Ideas, Selections WHERE userId = '".$userid."' AND Selections.ideaId =  Ideas.ideaId ORDER BY lastUpdateTime $limitString");
}
