...
while($item = dbFetchObject($items)) {?>
	<tr>
		<td class="headcol"><img
			src="<?= $serverUrl . $uiRoot ?>innoworks/retrieveImage.php?action=userImg&actionId=<?= $userId ?>"
			style="width: 2em; height: 2em;" /> <?= getUserInfo($userId)->username ?><br />
			<span style="font-size: 0.8em;"> <? if(isset($item->groupId)) { echo getGroupDetails($item->groupId)->title; } ?>
		</span>
		</td>
