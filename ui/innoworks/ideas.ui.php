...
<?=$idea->title?> <a href="javascript:showDetails('ideadetails_form_<?= $idea->ideaId?>')">Mission</a>
...
<!-- <a href="javascript:dijit.byId('ideasPopup').show();">Review</a> -->
...
<form id="ideadetails_form_<?= $idea->ideaId?>" style="display: none;">
	<? renderGenericUpdateForm($ideas ,$idea, array("ideaId", "title","userId", "createdTime")); ?> <input
...
function renderIdeaFeatures($ideaId) { ?>
	<form id="addfeature_form_<?= $ideaId?>" class="addForm">
		<span> New Feature | </span>
		<? renderGenericAddForm("Features", array("featureId", "ideaId")); ?>
		<input type="hidden" name="ideaId" value="<?= $ideaId?>"/>
		<input type="hidden" name="action" value="addFeature"/>
		<input type="button" onclick="genericAdd('addfeature_form_<?= $ideaId?>')" value=" + "/>
	</form>
	<?
...
function renderFeature($features, $feature) {?>
	<tr>
		...
		<td><input type="button" onclick="genericDelete('deleteFeature','<?= $feature->featureId ?>')"  value=" - "/></td>
	</tr>
<?}

function renderIdeaRoles($ideaId) { ?>
	<form id="addrole_form_<?= $ideaId?>" class="addForm">
		<span> New Role | </span>
		<? renderGenericAddForm("Roles", array("roleId", "ideaId"));	?>
		<input type="hidden" name="ideaId" value="<?= $ideaId?>"/>
		<input type="hidden" name="action" value="addRole"/>
		<input type="button" onclick="genericAdd('addrole_form_<?= $ideaId?>')" value=" + "/>
	</form>
...
function renderRole($roles, $role) {?>
	<tr>
		...
		<td><input type="button" onclick="genericDelete('deleteRole','<?= $role->roleId ?>')"  value=" - "/></td>
	</tr>
<?}
